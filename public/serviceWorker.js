const WINK_NEXTCHAT_CACHE = "wink-nextchat-cache";
const WINK_NEXTCHAT_FILE_CACHE = "wink-nextchat-file";
...
self.addEventListener("install", function (event) {
  ...
  event.waitUntil(
    caches.open(WINK_NEXTCHAT_CACHE).then(function (cache) {
      return cache.addAll([]);
    }),
  );
});
...
async function upload(request, url) {
  ...
  const cache = await caches.open(WINK_NEXTCHAT_FILE_CACHE)
